...
def get_preds_lin_reg(df, N, pred_min, offset):
    ...
    for i in range(offset, len(df["stockClose"])):
        ...
        pred_list.append(pred[0][0])  # Predict the close price using the model
...
cursor = db.cursor()
query = "select d.stockDate,d.stockOpen,d.stockHigh,d.stockLow,d.stockClose,d.adjClose,d.volume,i.stockName from " \
        "stockdetails d join stockinfo i on d.stockId = i.stockId where stockName = 'twtr' "
df = pd.read_sql_query(query, db)
df.loc[:, "stockDate"] = pd.to_datetime(df["stockDate"], format="%Y-%m-%d")

# Get month of each sample
#df["hour"] = df["stockDate"].dt.day
#print(df.head)
...
ax = test.plot(x="stockDate", y="est_N5", style='rx-', grid=True)
# ax.legend(['test', 'predictions using linear regression'], loc='upper left')
ax.legend(['Predicted Values'], loc='upper left')
ax.set_xlabel("date")
ax.set_ylabel("USD")
#plt.show()
test_lin_reg = test
